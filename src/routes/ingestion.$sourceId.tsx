...
export const Route = createFileRoute("/ingestion/$sourceId")({
  head: ({ params }) => ({
    meta: [{ title: `${params.sourceId} — Ingestion — Signal` }],
  }),
    ...
    <SignalShell>
      <div className="px-6 py-6 max-w-5xl">
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          ...
          <Tile label="Pushed to staging" value={String(totalPushed)} accent />
        </div>
          ...
          <section className="rounded-lg border bg-card">
            <header className="px-5 py-3 border-b">
              ...
              <p className="text-xs text-muted-foreground">
                What was scanned, what became a signal, and what got filtered before staging.
              </p>
            </header>
...
function Tile({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-lg border bg-card px-4 py-3">
      ...
      <div
        className={`font-mono tabular-nums text-xl mt-0.5 ${accent ? "text-primary" : ""}`}
      >
        {value}
      </div>
    </div>
