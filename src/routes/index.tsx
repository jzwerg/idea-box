...
function SignalDashboard() {
  ...
  const { weights, registerApply, runs: agentRuns, status: agentStatus, proposeRule } = useAgent();
  const {
    views,
    activeViewId,
    pinned,
  ...
  const bulkDismiss = () => {
    ...
    // Propose for first as representative
  ...
  // ------ visible: filter -> view-match -> sort (pinned, manualRank, score) ------
  const visible = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      ...
      // Manual rank next (lower idx = higher)
    ...
    return list;
  }, [requests, tab, areaFilter, userTypeFilter, query, weights, activeView, pinned, manualRank, tags, notes]);
  ...
  return (
    ...
    >
      ...
      <div className="border-b bg-primary/[0.04] px-6 py-2 flex items-center justify-between gap-3 text-xs">
        ...
        <Link to="/agent" className="text-primary hover:underline font-medium">
          Tune & run →
        </Link>
      </div>
            ...
            <thead className="text-xs uppercase tracking-wider text-muted-foreground border-b">
              <tr>
                ...
                <th className="w-8 px-2 py-2.5 text-left">
                  {tab === "new" && <Checkbox checked={allVisibleChecked} onCheckedChange={toggleAll} />}
                </th>
